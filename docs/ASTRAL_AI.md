...
- Нет фонового rollup, если `ai_metrics_on_demand=true`.
...
- Рендеринг **детерминированный**: UI рисует графики локально (spec), либо делает PNG из canvas (image).
- Для spec используется та же модель, что в `ai_model` (отдельной модели не требуется).
- По умолчанию используется **spec** (минимальная нагрузка). Image‑режим опционален.
