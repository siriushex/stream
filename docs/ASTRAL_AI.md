...
- Метрики считаются **только по запросу** (on‑demand).
- Фоновый rollup **принудительно выключен** (даже если настройка была изменена).
- `ai_metrics_retention_days` принудительно = `0` (метрики не сохраняются).
...
Примечание: по умолчанию логи **не** добавляются (минимальная нагрузка).
Логи включаются автоматически, если запрос про ошибки/проблемы, либо явно через `include_logs=true`.
...
### Принципы минимальной нагрузки
- Метрики **всегда** on‑demand (без фонового rollup).
- Логи ограничены лимитом, CLI запуски дозированы (1 параллельный).
- Графики строятся из on‑demand метрик (без фоновых задач).

## Модель OpenAI по умолчанию и fallback
- Если `ai_model` пустой, используется **`gpt-5-nano`** по умолчанию (самый экономичный).
- При ошибке `model_not_found` или неподдерживаемом `response_format` выполняется fallback по цепочке:
  - для `gpt-5-nano`: `gpt-5-mini` -> `gpt-4.1`
  - для `gpt-5-mini`: `gpt-5-nano` -> `gpt-4.1`
- Для UI это означает, что можно оставить поле модели пустым, и система выберет безопасный дефолт.
Примечание: для `gpt-5`, `gpt-5-mini`, `gpt-5-nano` параметр `temperature` не отправляется (совместимость API).

## Диаграммы и графики (AI)
- Если запрос требует графики/диаграммы, AI возвращает поле `charts` (line/bar + series).
- При запросах на графики AI автоматически получает **on‑demand метрики** (логи + runtime snapshot).
- Рендеринг **детерминированный**: UI рисует графики локально (spec).
- Для spec используется та же модель, что в `ai_model` (отдельной модели не требуется).
- Используется **только spec‑режим** (минимальная нагрузка).
...
- При on‑demand режиме сводка строится из логов + runtime snapshot (без фонового rollup).
...
- Контекст (логи/CLI) подбирается автоматически для минимальной нагрузки.
