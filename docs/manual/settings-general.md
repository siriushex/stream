...
Это режим, когда Stream Hub распределяет каналы по нескольким процессам на разных портах.
Смысл: если одно ядро CPU упирается в 100% (часто при SoftCAM), несколько процессов помогают разгрузить CPU.

Поля:

- **Enable stream sharding** — включить/выключить.
- **Shard port base** — базовый порт (например `9060`).
- **Shard count** — сколько шард‑процессов.
