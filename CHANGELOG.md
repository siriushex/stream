...
- Changes:
  ...
  - Stream/Adapter/Splitter/Buffer Save: also defer config export/snapshots to the async exporter (primary writer only) to reduce Save-induced stalls under load.
  ...
  - Export async: also use `ionice` when available to reduce IO contention during background exports.
...
- Changes:
  ...
  - Inputs: auto-enable `playout` (carrier + NULL stuffing) for `net_profile=max`/`superbad` to keep outputs continuous on bursty/stalling HTTP-TS/HLS sources.
...
- Changes:
  ...
  - Docs: document `superbad` and scheduled autotune workflow.
- Tests:
  - `./configure.sh && make`
  - `python3 -m py_compile tools/net_autotune.py`
### 2026-02-08
...
### 2026-02-07
- Changes:
  - Inputs: add aggressive `max` net_profile and allow it in settings/input selectors.
  - Inputs: allow `#net_auto_*` tuning (burst/relax/window/min-interval) and fix duplicate auto-init/escalate calls.
  - UI: add net_auto advanced fields and max jitter defaults in Input Resilience settings.
- Tests:
  - Not run (UI/Lua changes).
### 2026-02-07
- Changes:
  ...
  - Inputs: auto-calc jitter buffer memory limit for profile-enabled inputs using assumed bitrate defaults.
  - Inputs: add adaptive auto-tune mode (`#net_auto=1`) for unstable HTTP/HLS sources (dynamic timeouts/backoff).
  - UI: add Settings → Inputs card “Input Resilience” (enable profiles + default profile + jitter/HLS defaults).
