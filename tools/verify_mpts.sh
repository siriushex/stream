...
EXPECT_PAT_NIT_PID="${EXPECT_PAT_NIT_PID:-}"
EXPECT_NIT_TS_LIST="${EXPECT_NIT_TS_LIST:-}"
...
if [[ -n "$EXPECT_PMT_PIDS" ]]; then
  IFS=',' read -r -a PMT_PID_LIST <<< "$EXPECT_PMT_PIDS"
  for entry in "${PMT_PID_LIST[@]}"; do
    entry_trim="$(echo "$entry" | xargs)"
    if [[ -z "$entry_trim" ]]; then
      continue
    fi
    sid="${entry_trim%%=*}"
    value="${entry_trim#*=}"
    if ! grep -q "PAT: pid: ${value} PMT pnr: ${sid}" "$LOG_FILE"; then
      echo "PAT PMT PID mismatch for pnr ${sid} (expected ${value})"
      exit 1
    fi
  done
fi

if [[ -n "$EXPECT_PMT_ES_PIDS" ]]; then
  IFS=';' read -r -a PMT_ES_LIST <<< "$EXPECT_PMT_ES_PIDS"
  for entry in "${PMT_ES_LIST[@]}"; do
    entry_trim="$(echo "$entry" | xargs)"
    if [[ -z "$entry_trim" ]]; then
      continue
    fi
    sid="${entry_trim%%=*}"
    value="${entry_trim#*=}"
    if ! grep -q "PMT: es_pids: pnr=${sid} list=${value}" "$LOG_FILE"; then
      echo "PMT ES PID list mismatch for pnr ${sid} (expected ${value})"
      exit 1
    fi
  done
fi
...
if [[ -n "$EXPECT_PAT_NIT_PID" ]]; then
  if ! grep -q "PAT: pid: ${EXPECT_PAT_NIT_PID} NIT" "$LOG_FILE"; then
    echo "PAT NIT PID mismatch (expected ${EXPECT_PAT_NIT_PID})"
    exit 1
  fi
fi

if [[ -n "$EXPECT_NIT_TS_LIST" ]]; then
  if ! grep -q "NIT: ts_list: ${EXPECT_NIT_TS_LIST}" "$LOG_FILE"; then
    echo "NIT ts_list mismatch (expected ${EXPECT_NIT_TS_LIST})"
    exit 1
  fi
fi
