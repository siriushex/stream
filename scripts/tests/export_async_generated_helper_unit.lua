...
local cfg = {
  ...
  data_dir = "/tmp/export_async_generated_helper_unit",
  db_path = "/tmp/export_async_generated_helper_unit.db",
}
local ok = mod.request({
  primary_path = "/tmp/export_async_generated_helper_unit.json",
}, cfg)
assert_true(ok == true, "request failed")
assert_true(captured and type(captured.argv) == "table", "spawn not called")

local helper_path = "/tmp/export_async_generated_helper_unit/.stream-export-write.lua"
...
assert_true(captured.argv[2] == helper_path,
  "expected generated helper path, got: " .. tostring(captured.argv[2]))
assert_true(type(files[helper_path]) == "string" and files[helper_path] ~= "",
  "generated helper content should be written")
assert_true(files[helper_path]:find("generated by export_async.lua", 1, true) ~= nil,
  "generated helper marker missing")
assert_true(captured.opts and captured.opts.cwd == cfg.data_dir,
  "expected worker cwd=data_dir, got: " .. tostring(captured.opts and captured.opts.cwd))

restore()
log.info("[unit] export_async_generated_helper_unit ok")
astra.exit()
