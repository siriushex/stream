...
if [[ "$HOST" == *"178.212.236.6"* ]] && [[ "${STREAM_ALLOW_PROD_DEPLOY:-}" != "1" ]]; then
  echo "[deploy] REFUSED: ${HOST} is production (178.212.236.6)."
  echo "[deploy] DEV/TEST deploys must target 178.212.236.2."
  echo "[deploy] If you really must deploy to PROD, set STREAM_ALLOW_PROD_DEPLOY=1 explicitly."
  exit 2
fi
