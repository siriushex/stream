...
char *EZXML_NIL[] = { ((void *)0) };


ezxml_t ezxml_child(ezxml_t xml, const char *name)
{
    xml = (xml) ? xml->child : ((void *)0);
...
const char *ezxml_attr(ezxml_t xml, const char *attr)
{
    ...
    if (! xml || ! xml->attr) return ((void *)0);
    while (xml->attr[i] && (strcmp(attr, xml->attr[i]) != 0)) i += 2;
    if (xml->attr[i]) return xml->attr[i + 1];

    while (root->xml.parent) root = (ezxml_root_t)root->xml.parent;
    for (i = 0; root->attr[i] && (strcmp(xml->name, root->attr[i][0]) != 0); i++);
    if (! root->attr[i]) return ((void *)0);
    while (root->attr[i][j] && (strcmp(attr, root->attr[i][j]) != 0)) j += 3;
    return (root->attr[i][j]) ? root->attr[i][j + 1] : ((void *)0);
}


ezxml_t ezxml_vget(ezxml_t xml, va_list ap)
{
    char *name = __builtin_va_arg(ap,char *);
    int idx = -1;
...
const char **ezxml_pi(ezxml_t xml, const char *target)
{
    ...
    while (root->xml.parent) root = (ezxml_root_t)root->xml.parent;
    while (root->pi[i] && (strcmp(target, root->pi[i][0]) != 0)) i++;
...
ezxml_t ezxml_err(ezxml_root_t root, char *s, const char *err, ...)
{
    ...
    snprintf(fmt, 128, "[error near line %d]: %s", line, err);

    __builtin_va_start(ap,err);
    vsnprintf(root->err, 128, fmt, ap);
    __builtin_va_end(ap);
...
char *ezxml_decode(char *s, char **ent, char t)
{
    ...
    for (s = r; ; ) {
        while (*s && *s != '&' && (*s != '%' || t != '%') && !((*__ctype_b_loc ())[(int) ((*s))] & (unsigned short int) _ISspace)) s++;
        ...
        else if ((t == ' ' || t == '*') && ((*__ctype_b_loc ())[(int) ((*s))] & (unsigned short int) _ISspace)) *(s++) = ' ';
        else s++;
    }
...
void ezxml_char_content(ezxml_root_t root, char *s, size_t len, char t)
{
    ...
    if (xml->txt != m) ezxml_set_flag(xml, 0x40);
}
...
void ezxml_proc_inst(ezxml_root_t root, char *s, size_t len)
{
    ...
    if (! strcmp(target, "xml")) {
        if ((s = strstr(s, "standalone")) && ! strncmp(s + strspn(s + 10,
            "\t\r\n " "='\"") + 10, "yes", 3)) root->standalone = 1;
        return;
    }

    if (! root->pi[0]) *(root->pi = malloc(sizeof(char **))) = ((void *)0);
    ...
    if (! root->pi[i]) {
        ...
        root->pi[i][0] = target;
        root->pi[i][1] = (char *)(root->pi[i + 1] = ((void *)0));
        root->pi[i][2] = strdup("");
    }
    ...
    for (s[len] = '\0'; s; ) {
        ...
        if (! *s) break;
            ...
            *(s = n + strcspn(n, "\t\r\n ")) = ';';
            ...
            ent = realloc(ent, (i + 3) * sizeof(char *));
            if (*c == '%') pe = ent;
            else root->ent = ent;
            ...
            *(++s) = '\0';
            ...
            while(1) {
                ...
                if (! root->attr[i]) {
                    ...
                    root->attr[i] = malloc(2 * sizeof(char *));
                    root->attr[i][0] = t;
                    root->attr[i][1] = (char *)(root->attr[i + 1] = ((void *)0));
                }
        ...
        else if (! strncmp(s, "<!--", 4)) s = strstr(s + 4, "-->");
        else if (! strncmp(s, "<?", 2)) {
            if ((s = strstr(c = s + 2, "?>")))
                ezxml_proc_inst(root, c, s++ - c);
        }
        else if (*s == '<') s = strchr(s, '>');
...
void ezxml_free_attr(char **attr) {
    ...
    if (! attr || attr == EZXML_NIL) return;
    while (attr[i]) i += 2;
    m = attr[i + 1];
    for (i = 0; m[i]; i++) {
        if (m[i] & 0x80) free(attr[i * 2]);
        if (m[i] & 0x40) free(attr[(i * 2) + 1]);
    }
    free(m);
    free(attr);
}
...
ezxml_t ezxml_parse_str(char *s, size_t len)
{
    ezxml_root_t root = (ezxml_root_t)ezxml_new(((void *)0));
    char q, e, *d, **attr, **a = ((void *)0);
    int l, i, j;

    root->m = s;
    if (! len) return ezxml_err(root, ((void *)0), "root tag missing");
    ...
    for (; ; ) {
        ...
        if (((*__ctype_b_loc ())[(int) ((*s))] & (unsigned short int) _ISalpha) || *s == '_' || *s == ':' || *s < '\0') {
            if (! root->cur)
                return ezxml_err(root, d, "markup outside of root element");

            s += strcspn(s, "\t\r\n " "/>");
            while (((*__ctype_b_loc ())[(int) ((*s))] & (unsigned short int) _ISspace)) *(s++) = '\0';
            ...
            for (l = 0; *s && *s != '/' && *s != '>'; l += 2) {
                ...
                s += strcspn(s, "\t\r\n " "=/>");
                if (*s == '=' || ((*__ctype_b_loc ())[(int) ((*s))] & (unsigned short int) _ISspace)) {
                    *(s++) = '\0';
                    q = *(s += strspn(s, "\t\r\n " "="));
                    if (q == '"' || q == '\'') {
                        attr[l + 1] = ++s;
            ...
            *s = '\0';
            ...
            if (((*__ctype_b_loc ())[(int) ((*s = q))] & (unsigned short int) _ISspace)) s += strspn(s, "\t\r\n ");
        }
        else if (! strncmp(s, "!--", 3)) {
...
ezxml_t ezxml_parse_fp(FILE *fp)
{
    ...
    if (! (s = malloc(1024))) return ((void *)0);
    do {
        len += (l = fread((s + len), 1, 1024, fp));
        if (l == 1024) s = realloc(s, len + 1024);
    } while (s && l == 1024);
...
ezxml_t ezxml_parse_fd(int fd)
{
    ...
    if ((m = mmap(((void *)0), l, 0x1 | 0x2, 0x02, fd, 0)) !=
        ((void *) -1)) {
        madvise(m, l, 2);
        root = (ezxml_root_t)ezxml_parse_str(m, st.st_size);
        madvise(m, root->len = l, 0);
    }
    else {
                      ...
                      size_t *max, short a)
{
    ...
    for (e = s + len; s != e; s++) {
        while (*dlen + 10 > *max) *dst = realloc(*dst, *max += 1024);
                    ...
                    size_t start, char ***attr)
{
    ...
    *s = (xml->child) ? ezxml_toxml_r(xml->child, s, len, max, 0, attr)
                      : ezxml_ampencode(xml->txt, -1, s, len, max, 0);
...
char *ezxml_toxml(ezxml_t xml)
{
    ezxml_t p = (xml) ? xml->parent : ((void *)0), o = (xml) ? xml->ordered : ((void *)0);
    ezxml_root_t root = (ezxml_root_t)xml;
...
void ezxml_free(ezxml_t xml)
{
    ...
    if ((xml->flags & 0x40)) free(xml->txt);
    if ((xml->flags & 0x80)) free(xml->name);
    free(xml);
}
...
ezxml_t ezxml_insert(ezxml_t xml, ezxml_t dest, size_t off)
{
    ...
    xml->next = xml->sibling = xml->ordered = ((void *)0);
    xml->off = off;
    xml->parent = dest;
    ...
    if ((head = dest->child)) {
        ...
        for (cur = head, prev = ((void *)0); cur && strcmp(cur->name, xml->name);
        ...
        if (cur && cur->off <= off) {
            ...
            if (prev && cur) prev->sibling = cur->sibling;
            xml->next = cur;
            for (cur = head, prev = ((void *)0); cur && cur->off <= off;
                 prev = cur, cur = cur->sibling);
            xml->sibling = cur;
...
ezxml_t ezxml_set_attr(ezxml_t xml, const char *name, const char *value)
{
    ...
    if (! xml->attr[l]) {
        ...
        if (xml->flags & 0x20) xml->attr[l + 3][c] = 0x80;
    }
    else if (xml->flags & 0x20) free((char *)name);

    for (c = l; xml->attr[c]; c += 2);
    if (xml->attr[c + 1][l / 2] & 0x40) free(xml->attr[l + 1]);
    if (xml->flags & 0x20) xml->attr[c + 1][l / 2] |= 0x40;
    else xml->attr[c + 1][l / 2] &= ~0x40;

    if (value) xml->attr[l + 1] = (char *)value;
    else {
        if (xml->attr[c + 1][l / 2] & 0x80) free(xml->attr[l]);
...
ezxml_t ezxml_cut(ezxml_t xml)
{
    ...
    if (! xml) return ((void *)0);
    if (xml->next) xml->next->sibling = xml->sibling;

    if (xml->parent) {
        cur = xml->parent->child;
        if (cur == xml) xml->parent->child = xml->ordered;
        else {
    ...
    xml->ordered = xml->sibling = xml->next = ((void *)0);
    return xml;
}
