...
# 98 "/opt/rh/devtoolset-2/root/usr/lib/gcc/x86_64-redhat-linux/4.8.2/include/stdarg.h" 3 4
...
static char oldfilename[4096];
static pthread_mutex_t logmutex = { { 0, 0, 0, 0, 0, 0, { 0, 0 } } };
...
void loginit(char *logname, logType_t type, int usePidInName)
{
    if(logname)
    {
        ...
        if(gUsePidInName)
            snprintf(curfilename, sizeof(curfilename), "/tmp/%s_%d.log", filename, (int)getpid());
        else
            snprintf(curfilename, sizeof(curfilename), "%s", filename);
        gLogtype = type;
        if(type & logType_file)
        {
            outfd = open(curfilename, 01 | 0100 | 01000, 0200 | 0400 | (0400 >> 3) | ((0400 >> 3) >> 3));
            if(outfd == -1)
            {
                fprintf(stderr, "%s %d: Failed(%s) create log file(%s)\n", __FUNCTION__, 47, strerror((*__errno_location ())), curfilename);
            }
            else
            {
...
static void renew_internal(int locked)
{
    ...
    {
        if(gUsePidInName)
            snprintf(oldfilename, sizeof(oldfilename), "/tmp/%s_%d.log" ".old", filename, (int)getpid());
        else
...
void logout(const char *mes, ...)
{
    ...
    if(gLogtype & logType_syslog)
    {
        ...
        if(strcasestr(mes, "failed") != ((void *)0))
            error = 1;
        else if(strcasestr(mes, "error") != ((void *)0))
            error = 1;
        else if(strcasestr(mes, "invalid") != ((void *)0))
            error = 1;

        __builtin_va_start(ap,mes);
        vsyslog(error ? 3 : 6, mes, ap);
        __builtin_va_end(ap);
    }
}
