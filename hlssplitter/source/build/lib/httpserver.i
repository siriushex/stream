...
extern int bindresvport6 (int __sockfd, struct sockaddr_in6 *__sock_in)
     __attribute__ ((__nothrow__));
# 482 "/usr/include/netinet/in.h" 3 4
struct in6_pktinfo
  {
...
static int worker_process_line(httpServerCallbackData_t *data, char *line, int line_number)
{
    ...
    if (line_number == 0)
    {
        ...
        if(valid_request)
        {
            ...
            for(index = 0; index < (sizeof(data->data.requestedResource) - 1) && (!((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISspace)) && *p; index++, p++)
    ...
    else
    {
        ...
        *p = '\0';
        ...
        else if(data->data.count_pairs < 20)
        {
            strncpy(data->data.pairs[data->data.count_pairs].key, tag, 50 - 1);
            strncpy(data->data.pairs[data->data.count_pairs].val, p, 1024 - 1);
            data->data.count_pairs++;
        }
        else
        {
            logout("%s %d: Failed to big header %d >= %d for %s : %s\n", (__func__), 223, data->data.count_pairs, 20, tag, p);
        }
    }
    return 1;
}
...
int putResponseToBuffer(char *outbuffer, int outsize, int http_version, int keepalive, int code, const char *cdesc)
{
    ...
    if(get_date_line(curtime, sizeof(curtime)) >= 0)
    {
        ...
        if(size >= 0)
        {
            result = snprintf((char*)outbuffer, outsize, response_header, http_version, code, cdesc, size, keepalive ? "Keep-Alive" : "Close", curtime, "STB100/BasicServer");
            if(result > 0)
            {
                if(code == 503)
                {
...
static void send_short_response(httpServerCallbackData_t *data, int code, const char *cdesc)
{
    ...
    if(data)
    {
        ...
        if(size > 0)
        {
            ...
            if(result != size)
                logout("%s %d: Failed send data %d != %d: %s\n", (__func__), 306, result, size, strerror((*__errno_location ())));
        }
        else
        {
...
static void setSocketOptions(httpServerCallbackData_t *data)
{
    ...
    one = 1;
    if(setsockopt(data->socket, 1, 9, &one, sizeof(one)) != 0)
        logout("%s %d: Failed set socket keepalive\n", (__func__), 321);

    one = 3;
    if(setsockopt(data->socket, 6, 6, &one, sizeof(one)) != 0)
        logout("%s %d: Failed set socket keepalive\n", (__func__), 325);

    one = 10;
    if(setsockopt(data->socket, 6, 4, &one, sizeof(one)) != 0)
        logout("%s %d: Failed set socket keepalive\n", (__func__), 329);

    one = 10;
    if(setsockopt(data->socket, 6, 5, &one, sizeof(one)) != 0)
        logout("%s %d: Failed set socket keepalive\n", (__func__), 333);
    ...
    if(data->server && data->server->timeout)
    {
        ...
        if(setsockopt(data->socket, 1, 20, (char *)&timeout,sizeof(timeout)) < 0)
            logout("%s %d: Setsockopt failed\n", (__func__), 340);

        if (setsockopt(data->socket, 1, 21, (char *)&timeout,sizeof(timeout)) < 0)
            logout("%s %d: Setsockopt failed\n", (__func__), 343);
    }
}
...
static void clientFunction(threadPoolFunctionData_t *ctx)
{
    ...
    if(ctx && ctx->ctx)
    {
        ...
        if(data->magic == 0x3EA59462)
        {
            ...
            do {
                ...
                if(data->data.secondstart)
                {
                    ...
                    {
                        logout("%s %d: Failed %d send %d bytes: %s\n", (__func__), 378, result, data->data.buffersize, strerror((*__errno_location ())));
                        result = -1;
                    }
                ...
                else
                {
                    ...
                    if(complete)
                    {
                        if((data->data.httpData.method == httpMethod_GET) || (data->data.httpData.method == httpMethod_HEAD))
                        {
                            ...
                            {
                                data->data.httpData.socket = data->socket;
            ...
            if(data->socket != -1)
            {
                ...
                if(result == 0)
                {
                    struct pollfd mfd = { data->socket, 0x001 | 0x002 | 0x008 | 0x010, 0 };
                    poll(&mfd, 1, 3000);
                }
        ...
        {
            logout("%s %d: Invalid magic 0x%X != 0x%X\n", (__func__), 456, data->magic, 0x3EA59462);
        }
    }
    else
    {
...
static void httpThreadFunction(threadFunctionData_t *ctx)
{
    ...
    if(ctx)
    {
        ...
        {
            if((data->magic == 0x67CE43B3) && (data->serverData->magic == 0x3E8C94A1))
            {
                do __builtin_memset (&cpuset, '\0', sizeof (cpu_set_t)); while (0);
                (__extension__ ({ size_t __cpu = (data->procnum); __cpu < 8 * (sizeof (cpu_set_t)) ? (((__cpu_mask *) ((&cpuset)->__bits))[((__cpu) / (8 * sizeof (__cpu_mask)))] |= ((__cpu_mask) 1 << ((__cpu) % (8 * sizeof (__cpu_mask))))) : 0; }));
                result = pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset);
                if(result != 0)
                    logout("%s %d: Failed set affinity\n", (__func__), 486);

                result = fcntl(data->socket, 3, 0);
                if(result != -1)
                {
                    result = fcntl(data->socket, 4, result| 04000);
                    if(result != -1)
                    {
                        ...
                        if(result == 0)
                        {
                            ...
                            while(ctx->needExecute)
                            {
                                if(userCallback == 0)
                                {
                                    ...
                                    if(userCallback == 0)
                                    {
                                        logout("%s %d: Failed get %d bytes of memory\n", (__func__), 506, (int)sizeof(httpServerCallbackData_t));
                                    }
                                }
                                ...
                                if(result > 0)
                                {
                                    ...
                                    if(result != -1)
                                    {
                                        if(userCallback)
                                        {
                                            ...
                                            if(threadPool_startInContext(data->threadPool, clientFunction, userCallback, "HttpClientFunc", threadPoolPriority_Normal, 0) == 0)
                                            {
                                        ...
                                        {
                                            logout("%s %d: Failed alloc %d bytes of memory\n", (__func__), 544, (int)sizeof(httpServerCallbackData_t));
                                            shutdown(result, SHUT_RDWR);
                                            close(result);
                                        }
                                    }
...
static int createListenSocket(httpServerMainThread_t *context)
{
    ...
    if(context && context->serverData)
    {
        ...
        if(sock != -1)
        {
            if(setsockopt(sock, 1, 2, &yes, sizeof(yes)) == -1)
            {
                logout("%s %d: Failed(%s) setsockopt\n", (__func__), 621, strerror((*__errno_location ())));
            }
            if(setsockopt(sock, 1, 15, &yes, sizeof(yes)) == -1)
            {
                logout("%s %d: Failed(%s) setsockopt\n", (__func__), 625, strerror((*__errno_location ())));
            }

            if(context->serverData->timeout >> 1)
            {
                timeout = (context->serverData->timeout >> 1);
                if(setsockopt(sock, 6, 9, &timeout, sizeof(timeout)) == -1)
                {
                    logout("%s %d: Failed(%s) setsockopt\n", (__func__), 633, strerror((*__errno_location ())));
                }
            }
        ...
        {
            logout("%s %d: Failed(%s) create socket\n", (__func__), 664, strerror((*__errno_location ())));
        }
    }
    else
    {
    ...
    if(server && interface && callback)
    {
        ...
        if(processes > 0)
        {
            ...
            if(newServer)
            {
                ...
                setreuid(0, 0);
                setregid(0, 0);
                for (iterator = (&newServer->threads)->next; iterator != (&newServer->threads); iterator = iterator->next)
                {
                    thread = ((httpServerMainThread_t *)((char *)(iterator)-(unsigned long)(&((httpServerMainThread_t *)0)->list)));
            ...
            {
                logout("%s %d: Failed alloc memory %d bytes\n", (__func__), 769, (int)sizeof(httpServerPrivate_t));
                result = 12;
            }
        }
        else
...
int stop_http_server(httpServer_t server)
{
    ...
    if(newServer)
    {
        if(newServer->magic == 0x3E8C94A1)
        {
            for (iterator = (&newServer->threads)->next; iterator != (&newServer->threads); iterator = iterator->next)
            {
                httpServerMainThread_t *thread = ((httpServerMainThread_t *)((char *)(iterator)-(unsigned long)(&((httpServerMainThread_t *)0)->list)));
                if(thread->thread)
                {
                    int status;
...
int process_http_connection(httpClientCtx_t *httpData, struct iovec *data, unsigned int datacount)
{
    ...
    if(httpData && data && datacount)
    {
        ...
        if(size <= 0x10000)
        {
            if((httpData->socket != -1) && httpData->instance && (httpData->instance->magic == 0x67CE43B3))
            {
                if(httpData->instance->serverData && httpData->instance->serverData->magic == 0x3E8C94A1)
                {
                    userCallback = threadPool_getContext(httpData->instance->threadPool, sizeof(httpServerCallbackData_t));
                    if(userCallback == 0)
                    {
                        logout("%s %d: Failed get %d bytes of memory\n", (__func__), 855, (int)sizeof(httpServerCallbackData_t));
                    }
                    else
                    {
                        memset(userCallback, 0, sizeof(*userCallback));
                        ...
                        if(threadPool_startInContext(httpData->instance->threadPool, clientFunction, userCallback, "HttpClientFunc", threadPoolPriority_Normal, 0) == 0)
                        {
                            result = 0;
                        }
                        else
