...
- Внутри workers:
  ...
  - `sendmmsg` для отправки батчами в несколько outputs (fast-path для 1316),
  - лимит обработки “за один epoll event” (fairness), чтобы один горячий сокет не вызывал джиттер у остальных,
...
```json
{
  "settings": {
    ...
    "performance_passthrough_worker_policy": "hash|least_loaded",
...
## 5.1) Auto‑режим: Watchdog + Blacklist (устойчивость)

Проблема: иногда поток выглядит “UDP→UDP passthrough”, но реально UDP датаграммы не содержат TS188
(например RTP encapsulation или “битый” источник). В таком случае dataplane может “принимать” UDP,
но не выдавать корректный TS.

Решение в `performance_passthrough_dataplane="auto"`:
- После старта dataplane поток попадает в список “pending” (внутренний `runtime.dp_watchdog_pending`).
- Каждые `PASSTHROUGH_DP_WATCHDOG_INTERVAL_SEC` секунд watchdog проверяет *только pending* потоки.
- Если видим:
  - `datagrams_in` растёт,
  - `bytes_out == 0`,
  - `bad_datagrams > 0` после небольшого grace‑периода,
  → делаем **fallback в legacy pipeline** и ставим **blacklist** на `PASSTHROUGH_DP_WATCHDOG_COOLDOWN_SEC`
    (по `stream_id + config_hash`), чтобы избежать рестарт‑петли “dataplane → fallback → dataplane”.

Это делает auto‑режим предсказуемым: “быстрый путь” включается только там, где он реально совместим.

Smoke‑проверка (Linux):
- `tools/tests/dataplane_watchdog_smoke.sh` (стартует Stream, шлёт bad UDP и ждёт fallback по логам).
