...
STREAM_ID="${STREAM_ID:-transcode_ladder_economical_hls_publish}"
SRC_ID="${SRC_ID:-src_udp}"
TEMPLATE_FILE="${TEMPLATE_FILE:-$ROOT_DIR/fixtures/transcode_ladder_economical_hls_publish.json}"
...
print("ladder_single_process:", info.get("ladder_single_process"))
...
echo "smoke_transcode_ladder_economical_hls_publish: group=$MC_GROUP in=$IN_PORT port=$PORT" >&2
...
echo "smoke_transcode_ladder_economical_hls_publish: ok" >&2
