...
# Give the Lua runtime a short moment to finish init (API routes, tools discovery, etc.).
sleep 0.5
...
for _ in $(seq 1 40); do
  set +e
  TOOLS_JSON="$(curl -fsS "http://127.0.0.1:${PORT}/api/v1/tools" "${AUTH_ARGS[@]}" 2>/dev/null)"
  CODE=$?
  set -e
  if [[ "$CODE" -eq 0 ]] && [[ -n "$TOOLS_JSON" ]]; then
    TOOLS_OK=1
    break
  fi
  sleep 0.5
...
if [[ "$TOOLS_OK" -ne 1 ]]; then
  echo "/api/v1/tools not ready (port=$PORT)" >&2
  dump_debug
  exit 1
fi
...
EMBED_BODY="$(curl -fsS "$EMBED_URL" "${AUTH_ARGS[@]}")"

EXPECTED_SRC="var src = \"/hls/${STREAM_ID}/index.m3u8\";"
if ! grep -q "$EXPECTED_SRC" <<<"$EMBED_BODY"; then
  echo "Embed page missing expected HLS src ($EMBED_URL)" >&2
  echo "$EXPECTED_SRC" >&2
  echo "$EMBED_BODY" | head -n 80 >&2 || true
  dump_debug
  exit 1
fi

if grep -q 'var src = ""' <<<"$EMBED_BODY"; then
  echo "Embed page has empty HLS src ($EMBED_URL)" >&2
  echo "$EMBED_BODY" | head -n 80 >&2 || true
  dump_debug
  exit 1
fi
